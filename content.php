...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	...
	if (has_post_thumbnail()) {
		...
		echo '<a href="' . get_permalink() . '" title="' . __('Read ', 'nerdyowl') . get_the_title() . '" rel="bookmark">';
	...
	?>
		<header class="entry-header">
			...
			<?php
				if($wp_query ->current_post == 0 && !is_paged() && is_front_page() ){
					echo 'This is the first post on the front page.';
				}
			?>
		...
		<div class="entry-content">
			...
			the_content( __( '', 'nerdyowl' ) );
			echo '</div>';
			echo '<footer class="entry-footer continue-reading">';
			echo '<a href="' . get_permalink() . '" title="' . __('Read ', 'nerdyowl') . get_the_title() . '" rel="bookmark">Read the article<i class="fa fa-arrow-circle-o-right"></i></a>';
			...
			<?php echo '<a href="' . get_permalink() . '" title="' . __('Continue Reading ', 'nerdyowl') . get_the_title() . '" rel="bookmark">Continue Reading<i class="fa fa-arrow-circle-o-right"></i></a>'; ?>
			</footer><!-- .entry-footer -->
			<?php } ?>

		</div><!-- .entry-content -->

	</div> <!--  closing of the 'index-box' div. -->
</article><!-- #post-## -->
